...
/*   Updated: 2024/05/07 00:25:23 by abenamar         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
Bureaucrat::Bureaucrat(std::string const &name, int const grade) throw(Bureaucrat::GradeTooHighException, Bureaucrat::GradeTooLowException) : name(name), grade(grade)
{
	...
	if (this->grade < 1)
	{
		o << "Bureaucrat::GradeTooHighException: " << *this << ": Grade is too high (smaller than 1)";
	...
	if (this->grade > 150)
	{
		o << "Bureaucrat::GradeTooLowException: " << *this << ": Grade is too low (greater than 150)";
...
void Bureaucrat::executeForm(AForm const &form) const throw()
{
	try
	{
		form.execute(*this);
